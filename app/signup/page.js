...
export default function SignupPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    const { email, password, targetDays } = formData;

    if (!email || !password || !targetDays) {
      setError("All fields are required");
      setLoading(false);
      return;
    }

    if (parseInt(targetDays, 10) < 1) {
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <div
          className={`absolute top-1/4 -left-32 w-96 h-96 ${t.decorative1} rounded-full blur-3xl transition-colors duration-300`}
        />
        <div
          className={`absolute bottom-1/4 -right-32 w-96 h-96 ${t.decorative2} rounded-full blur-3xl transition-colors duration-300`}
        />
      </div>
        ...
        >
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="space-y-2">
              ...
              />
              <p
                className={`text-xs ${t.helperText} transition-colors duration-300`}
              >
